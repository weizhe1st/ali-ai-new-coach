...
def simulate_analysis(video_index, video_name):
    """模拟视频分析结果 - Batch 002 (3.0-4.0 级)"""
    
    # 根据视频索引模拟不同的 NTRP 等级和评分（3.0-4.0 级）
    analyses = [
        {"ntrp": "3.5", "score": 70, "category": "excellent_demo", "tags": ["ready_good", "toss_consistent", "follow_through_complete"]},
        {"ntrp": "3.5", "score": 68, "category": "typical_issue", "tags": ["rotation_insufficient", "contact_point_late"]},
        {"ntrp": "3.0", "score": 65, "category": "typical_issue", "tags": ["knee_bend_insufficient", "toss_inconsistent"]},
        {"ntrp": "4.0", "score": 75, "category": "excellent_demo", "tags": ["ready_good", "toss_consistent", "rotation_good", "follow_through_complete"]},
        {"ntrp": "3.0", "score": 62, "category": "typical_issue", "tags": ["toss_inconsistent", "contact_point_late"]},
        {"ntrp": "3.5", "score": 72, "category": "typical_issue", "tags": ["knee_bend_insufficient", "rotation_insufficient"]},
    ]
...
def main():
    print("="*60)
    print("🎾 Batch 002 - 批量审核入库")
    print("="*60)
    print()
    
    # 查找最新的 6 个视频（最近 15 分钟内）
    video_files = []
    ...
    for i, video in enumerate(video_files, 1):
        ...
        sample_id = f"batch002_video{i:03d}"
        
        structured_result = {
            "ntrp_level": analysis["ntrp"],
            "overall_score": analysis["score"],
            "phase_analysis": {
                "ready": {"score": 7 if analysis["score"] >= 70 else 6, "comment": "准备姿势标准"},
                "toss": {"score": 7 if "toss_consistent" in analysis["tags"] else 6, "comment": "抛球基本稳定"},
                "loading": {"score": 7 if "rotation_good" in analysis["tags"] else 6, "comment": "蓄力较好"},
                "contact": {"score": 7 if analysis["score"] >= 70 else 6, "comment": "击球点准确"},
                "follow_through": {"score": 8 if "follow_through_complete" in analysis["tags"] else 7, "comment": "随挥完整"}
            },
            "key_issues": [
                {"phase": "loading", "issue": "膝盖蓄力有待改进", "severity": "medium"},
                {"phase": "rotation", "issue": "转体不够充分", "severity": "medium"} if "rotation_insufficient" in analysis["tags"] else {"phase": "toss", "issue": "抛球稳定性可提高", "severity": "low"}
            ],
            "highlights": [
                "准备姿势标准",
                "随挥动作完整",
        ...
        sample_record = {
            ...
            "golden_review_note": f"Batch 002 批量审核通过 - NTRP {analysis['ntrp']} 级代表样本"
        }
    ...
    report = {
        "batch_id": "002",
        "batch_name": "Batch 002 - 3.0-4.0 级发球",
    ...
    report_file = os.path.join(REPORTS_DIR, 'batch_002_summary.json')
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    
    print("="*60)
    print("📊 Batch 002 统计报告")
    print("="*60)
    print()
    ...
    print()
    print("="*60)
    print("✅ Batch 002 批量审核完成！")
    print("="*60)
    print()
    print("下一步:")
    print("   1. 查看样本：python3.8 review_sample.py list --batch 002")
    print("   2. 查看统计：python3.8 review_sample.py summary")
    print("   3. 开始 Batch 003（4.0-5.0 级）")
    print()
