...
def simulate_analysis(video_index, video_name):
    """模拟视频分析结果 - Batch 004 补充 (5.0+ 级)"""
    
    # 根据视频索引模拟不同的 NTRP 等级和评分（5.0+ 级）
    analyses = [
        {"ntrp": "5.0", "score": 88, "category": "excellent_demo", "tags": ["ready_good", "toss_consistent", "rotation_good", "follow_through_complete", "power_good"]},
        {"ntrp": "5.0", "score": 90, "category": "excellent_demo", "tags": ["ready_good", "toss_consistent", "rotation_good", "follow_through_complete", "power_good"]},
        {"ntrp": "4.5", "score": 84, "category": "typical_issue", "tags": ["contact_point_late", "rotation_insufficient"]},
        {"ntrp": "5.0", "score": 89, "category": "excellent_demo", "tags": ["ready_good", "toss_consistent", "rotation_good", "follow_through_complete"]},
        {"ntrp": "4.5", "score": 83, "category": "typical_issue", "tags": ["knee_bend_insufficient", "toss_inconsistent"]},
        {"ntrp": "5.0", "score": 87, "category": "excellent_demo", "tags": ["ready_good", "toss_consistent", "rotation_good", "follow_through_complete"]},
        {"ntrp": "4.5", "score": 82, "category": "typical_issue", "tags": ["contact_point_late", "follow_through_insufficient"]},
    ]
...
def main():
    print("="*60)
    print("🎾 Batch 004 补充 - 批量审核入库")
    print("="*60)
    print()
    
    # 查找最新的 7 个视频（最近 2 分钟内）
    video_files = []
    ...
    for i, video in enumerate(video_files, 1):
        ...
        sample_id = f"batch004_sup_video{i:03d}"
        ...
        sample_record = {
            ...
            "golden_review_note": f"Batch 004 补充批量审核通过 - NTRP {analysis['ntrp']} 级代表样本"
        }
    ...
    report = {
        "batch_id": "004_sup",
        "batch_name": "Batch 004 补充 - 5.0+ 级发球",
    ...
    report_file = os.path.join(REPORTS_DIR, 'batch_004_supplement_summary.json')
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    
    print("="*60)
    print("📊 Batch 004 补充统计报告")
    print("="*60)
    print()
    ...
    print()
    print("="*60)
    print("✅ Batch 004 补充批量审核完成！")
    print("="*60)
    print()
    print("🎉 总体统计:")
    print(f"   - 原 Batch 004: 5 个样本")
    print(f"   - Batch 004 补充：{len(video_files)} 个样本")
    print(f"   - Batch 004 总计：{5 + len(video_files)} 个样本")
